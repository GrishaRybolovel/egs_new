              ...
              child: FutureBuilder<List<Document>?>(
                ...
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    ...
                    return Text('Документов не найдено.');
                  } else {
                    List<DataRow> rows = snapshot.data!.where((document) {
                      ...
                      if (widget.initialProject == null){
                        ...
                        print(document.projects!.contains(widget.initialProject?.id) ?? false);
                        return document.projects!.contains(widget.initialProject?.id) ?? false;
                      }
                      ...
                      return DataRow(
                        cells: [
                          DataCell(ElevatedButton(
                            onPressed: () {
                              Provider.of<MenuAppController>(context,
                                  listen: false)
                                  .navigateTo(DocumentForm(document: document));
                            },
                          ...
                          DataCell(Text(document.duration
                              .toString()
                              .substring(0, 10))),
                          DataCell(ElevatedButton(
                            onPressed: () async {
